...
namespace OSDownloader
{
    public partial class Preferences : Window
    {
        ...
        public Preferences(string openInTabItemWithName)
        {
            InitializeComponent();

            if (openInTabItemWithName != null && openInTabItemWithName.Trim().Length > 0 )
                tabControl.SelectedItem = (TabItem)tabControl.FindName(openInTabItemWithName);

            // Set the state of window controls based on settings from the .config file

            RegistryKey registryKey = Registry.CurrentUser.OpenSubKey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", true);
            if (registryKey.GetValue("OSDownloader") == null || !Settings.Default.StartOnSystemStartup)
            {
        ...
        private void SaveSettings()
        {
            ...
            if (registryKey != null)
            {
                if (Settings.Default.StartOnSystemStartup)
                {
                    ...
                    registryKey.SetValue("OSDownloader", "\"" + Assembly.GetExecutingAssembly().Location + "\"");
                }
                else
                {
                    // Delete the value from the Registry to disable application start on system startup
                    registryKey.DeleteValue("OSDownloader", false);
                }
            }
